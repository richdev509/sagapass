...
@section('title', 'Vidéos rejetées')

@section('content')
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-2">Vidéos rejetées</h1>
            <p class="text-muted mb-0">Liste des vidéos refusées</p>
        </div>
        <div>
            <a href="{{ route('admin.video-verification.index') }}" class="btn btn-outline-warning me-2">
                <i class="fas fa-clock"></i> En attente
            </a>
            <a href="{{ route('admin.video-verification.approved') }}" class="btn btn-outline-success">
    ...
    <div class="card mb-4">
        <div class="card-body">
            <form method="GET" action="{{ route('admin.video-verification.rejected') }}" class="row g-3">
                <div class="col-md-4">
                    <label class="form-label">Recherche</label>
                    <input type="text" name="search" class="form-control" placeholder="Nom, email..." value="{{ request('search') }}">
                </div>
                <div class="col-md-3">
                    <label class="form-label">Date rejet (De)</label>
                    <input type="date" name="date_from" class="form-control" value="{{ request('date_from') }}">
                </div>
                <div class="col-md-3">
                    <label class="form-label">Date rejet (À)</label>
    ...
    <div class="card">
        <div class="card-header bg-danger text-white">
            <h5 class="mb-0"><i class="fas fa-times-circle"></i> Vidéos rejetées ({{ $users->total() }})</h5>
        </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <small class="text-danger">
                                        {{ Str::limit($user->video_rejection_reason ?? 'Non spécifiée', 50) }}
                                    </small>
                                </td>
                                <td>
